...
<div class="tab_cont clearfix">
  <div class="cont_ri" id="ri_cont">
    <div class="sale_box">
      ...
      <span id="box_tab">
		<div style="margin-top:20px; width: 50%;height:20px;float:left;text-align: center">
			余额：<strong><c:choose><c:when test="${result_code eq '0000'}">${balance}</c:when><c:otherwise>${result_msg}</c:otherwise></c:choose> </strong>
		</div>
		<div style="margin-top:20px; width: 50%;height:20px;float:left;text-align: center">
			保证金：<strong><c:choose><c:when test="${result_code eq '0000'}">${bailBalance}</c:when><c:otherwise>${result_msg}</c:otherwise></c:choose> </strong>
		</div>
