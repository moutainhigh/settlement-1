...
<c:set var="search_path"
       value="${ctx}/unspay/deductUpload?filename=${filename}&uploadStartDate=${uploadStartDate}&uploadEndDate=${uploadEndDate}"/>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            function loadUploadDetailData(filename) {
                ...
                $.get("${ctx}/unspay/uploadDeductDetail", {'filename': filename}, function (list) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        var amount = '';
                        var operate = '';
                        if(item.deductResult!=null){
                            if (item.deductResult == '0000')
                                status = '<td class="success">' + '成功' + '</td>';
                            else if (item.deductResult != '10' && item.deductResult != '0000')
                                status = '<td class="failur">' + item.desc + '</td>';
                            else if (item.deductResult == '10') {
                            ...
                            if (item.verifyStatus == '0')
                                ...
                                status = '<td class="refused">' + '待发送' + '</td>';
                        ...
                        var $item = $('<tr height="45">'
                                ...
                                + '<td>' + item.orderId + '</td>'
                                + '<td>' + (item.planDate == null ? "" : new Date(item.planDate).format('yyyy-MM-dd hh:mm')) + '</td>'
                                ...
                                + '<td>' + (item.deductResult==null?"":item.deductResult) + '</td>'
                                + status
                                + '<td>' + operate + '</td>'
                    ...
                    $upload_detail_table.find('button[data-orderId]').unbind('click').click(function () {
                        ...
                        $.get("${ctx}/unspay/queryDeductOrder", {'orderId': orderId}, function (unspayDeductResponse) {
                            var $tr = $upload_detail_table.find('button[data-orderId="'+orderId+'"]').parents('tr');
                            if(unspayDeductResponse.resultCode!='0000'){
                                alert('同步失败，'+unspayDeductResponse.resultMessage);
                                $tr.children(':eq(10)').html(unspayDeductResponse.resultCode);
                                $tr.children(':eq(11)').html(unspayDeductResponse.resultMessage);
                            }else{
                                $tr.children(':eq(10)').html(unspayDeductResponse.deductResult);
                                $tr.children(':eq(11)').html(unspayDeductResponse.desc);
                            }
            ...
            function verify(verifyStatus) {
                ...
                $.post("${ctx}/unspay/varifyDeduct/" + verifyStatus, {
                    'orderIds': orders,
                    'date': new Date()
                }, function (status) {
                    if (status != '1') {
...
<div class="tab_cont clearfix">
    <div class="cont_ri" id="ri_cont">
      ...
      <div class="condition_a">
          <ul class="clearfix">
              <form id="page_form" action="${ctx}/unspay/deductUpload" method="GET">
                  <li>
                      <label>文件名称</label>
              ...
              <li style="width:200px;float: right;">
                  ...
                  <form id="upload_form" method="post" action="${ctx}/unspay/deductUpload"
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="order_box" id="order_tab">
          ...
          <c:forEach begin="0" end="${page.pageSize-1}" items="${page.result}" var="unspayDeduct">
              <tr height="45">
                  <td class="click_td">${unspayDeduct.filename}</td>
                  <td><fmt:formatDate value="${unspayDeduct.uploadDate}" pattern="yyyy-MM-dd"></fmt:formatDate></td>
                  <td>${unspayDeduct.staffName}</td>
                  <td>${unspayDeduct.count}</td>
                  <td class="notaudited">${unspayDeduct.notVerify}</td>
                  <td class="refused">${unspayDeduct.refused}</td>
                  <td class="td_last"><strong class="success">${unspayDeduct.success}</strong> | <strong
                          class="failur">${unspayDeduct.failur}</strong> | <strong class="going">${unspayDeduct.process}</strong>
                  </td>
              </tr>
          </c:forEach>
