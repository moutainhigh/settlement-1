...
<c:set var="search_params"
       value="name=${name}&phoneNo=${phoneNo}&idCardNo=${idCardNo}&filename=${filename}&deductResult=${deductResult}&responseStartDate=${responseStartDate}&responseEndDate=${responseEndDate}"/>
<c:set var="search_path"
       value="${ctx}/unspay/deductList?${search_params}"/>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="tab_cont clearfix">
    <div class="cont_ri" id="ri_cont">
      ...
      <div class="condition_a">
          <form id="deduct_list_form" action="${ctx}/unspay/deductList" method="GET">
              <ul class="clearfix">
                  <li>
                      <label>姓名</label>
              ...
              <ul class="clearfix">
                  ...
                  <li>
                      <label>扣款结果</label>
                      <select name="deductResult" style="width:150px; margin:0 2px;" value="${deductResult}">
                          <option></option>
                          <option value="0000" <c:if test="${'0000' eq deductResult}">selected</c:if>>成&nbsp;&nbsp;功</option>
                          <option value="10" <c:if test="${'10' eq deductResult}">selected</c:if>>处理中</option>
                          <option value="20" <c:if test="${'20' eq deductResult}">selected</c:if>>失&nbsp;&nbsp;败</option>
                      </select>
                  </li>
                  <li>
                      <label>扣款时间</label>
                  ...
                  <li style="margin-left:0px;">
                      ...
                      <button id="download_deduct_list_btn" style="margin-left:20px;margin-right:50px;width:80px;"><a style="color: black;" href="${ctx}/unspay/deductDownload?${search_params}">导出</a></button>
                  </li>
              </ul>
          </form>
      </div>
      <table cellpadding="0" cellspacing="0" border="0" class="order_box" id="order_tab">
          <tr height="50">
              ...
              <th>状态码</th>
              <th>扣款结果</th>
              <th class="td_last">扣款目的</th>
          </tr>
          <c:forEach items="${page.result}" var="unspayDeduct">
              <tr height="45">
                  <td>${unspayDeduct.orderId}</td>
                  <td>${unspayDeduct.filename}</td>
                  <td>${unspayDeduct.loanApplyId}</td>
                  <td>${unspayDeduct.repayPhaseId}</td>
                  <td>${unspayDeduct.name}</td>
                  <td>${unspayDeduct.phoneNo}</td>
                  <td>${unspayDeduct.cardNo}</td>
                  <td><yh:bank bankCode="${unspayDeduct.bankCode}"/></td>
                  <td style="font-weight: bold;">
                      <fmt:formatNumber pattern="###,###,###" value="${unspayDeduct.amount}"></fmt:formatNumber><small><fmt:formatNumber pattern=".00" value="${unspayDeduct.amount % 1.0}"></fmt:formatNumber></small>
                  </td>
                  <td><fmt:formatDate value="${unspayDeduct.planDate}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></td>
                  <td><fmt:formatDate value="${unspayDeduct.sendDate}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></td>
                  <td><fmt:formatDate value="${unspayDeduct.responseDate}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></td>
                  <td>${unspayDeduct.deductResult}</td>
                  <td class="<c:choose><c:when test="${unspayDeduct.deductResult eq '0000'}">success</c:when>
                      <c:when test="${unspayDeduct.deductResult eq '10'}">going</c:when>
                      <c:otherwise>failur</c:otherwise>
                  </c:choose>">
                      <c:choose>
                          <c:when test="${unspayDeduct.deductResult eq '0000'}">
                              成功
                          </c:when>
                          <c:when test="${unspayDeduct.deductResult eq '10'}">
                              处理中
                          </c:when>
                          <c:otherwise>
                              ${unspayDeduct.desc}
                  ...
                  <td class="td_last">${unspayDeduct.purpose}</td>
              </tr>
          </c:forEach>
