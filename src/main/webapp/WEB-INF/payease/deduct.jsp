    ...
    <div class="cont_ri" id="ri_cont">
        <div class="sale_box">
            <div class="cont_ri_til clearfix">
                <div class="left_btn">
                    <ul id="tabs_ul">
                        <li class="cont">代扣列表</li>
                    </ul>
                </div>
            </div>
      <span id="box_tab">
      <div class="condition_a">
          <form action="${ctx}/payease/trs001008/list" method="post">
              <ul class="clearfix">
                  <li>
                      <label>姓名</label>
                  ...
                  <li>
                      <label>身份证号</label>
                      <input type="text" placeholder="身份证号" class="place_txt" style="width:200px;" name="id"
                             value="${id}"/>
                  </li>
                  <li style="margin-left:0px;">
                      <button id="upload_button" type="button" style="margin-left:50px;margin-right:20px;width:80px;">
                          导入
                      </button>
                      <a id="upload_templete" href="${ctx}/payease/templete?templete=首信易-代扣模板.xls"
          ...
          <form id="upload_form" action="${ctx}/payease/trs001008/upload" method="post" enctype="multipart/form-data">
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="order_box" id="order_tab">
          ...
          <c:forEach items="${page.result}" var="trs001008">
              <tr height="45">
                  <td>${trs001008.serlNum}</td>
                  <td>${trs001008.user}</td>
                  <td>${trs001008.accName}</td>
                  <td class="idCardNo">${trs001008.id}</td>
                  <td class="bankCardNo">${trs001008.accNum}</td>
                  <td>${trs001008.accBank}</td>
                  <td>${trs001008.amount}</td>
                  <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${trs001008.returnTime}"></fmt:formatDate></td>
                  <td class="td_last">${trs001008.returnMsg}</td>
              </tr>
          </c:forEach>
