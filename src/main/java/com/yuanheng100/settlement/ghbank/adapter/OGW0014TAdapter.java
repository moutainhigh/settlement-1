 ...
 */
public class OGW0014TAdapter extends AbstractGHBankAdapter<OGW0014TResp>
{
    private static Logger logger = LoggerFactory.getLogger(OGW0014TAdapter.class);

    
    @Override
    protected Document createXml(OGW0014TResp ogw0014TResp)
    {
        Document document = super.createXml(ogw0014TResp);
        Element rootElement = document.getRootElement();
        Element body = rootElement.element("body");
        Element xmlpara = body.element("XMLPARA");
        //将当前业务的字段添加到xmlpara，在父类中加密并替换成加密字符串

        xmlpara.addElement("RETURNCODE").addText(StringUtils.trimToEmpty(ogw0014TResp.getReturnCode()));
        xmlpara.addElement("RETURNMSG").addText(StringUtils.trimToEmpty(ogw0014TResp.getReturnCode()));
        xmlpara.addElement("OLDREQSEQNO").addText(StringUtils.trimToEmpty(ogw0014TResp.getOldReqSeqNo()));
